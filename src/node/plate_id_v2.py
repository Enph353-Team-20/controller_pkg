...
class PlateID():
    ...
    def findCorners(self, plate_img):
        ...
        hsv = cv2.cvtColor(plate_img.raw.copy(), cv2.COLOR_BGR2HSV)
    ...
    def perspective_transform_corners(self,plate_img,corners):

        dest_pts = np.array([(0, 0), (150,0), (150, 450), (0,450)])
        matrix = cv2.getPerspectiveTransform(np.float32(corners), np.float32(dest_pts))
        warped = cv2.warpPerspective(plate_img.raw.copy(), matrix, (150,450))
